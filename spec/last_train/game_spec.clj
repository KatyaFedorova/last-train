...
(describe "Starting a game"
  (let [{:keys [state output]} (game/start puzzles/reference)]
    (it "shows each passenger's boarding statement"
      (should-contain "Vera (A): \"There are no Agents on this train.\"" output)
      (should-contain "Tomasz (B): \"Vera is not an Agent.\"" output)
      (should-contain "Ilse (C): \"Tomasz and I are the same kind.\"" output)
      (should-contain "Mr. Grey (D): \"Tomasz and Ilse are the same kind.\"" output))

    (it "opens Station 1 with three questions"
      (should= 2 (:round state))
      (should= 3 (:questions-left state))
      (should-contain "Round 2 - Station 1" output)
      (should-contain "Questions left: 3" output))

    (it "knows the worlds left by the opening statements"
      (should= 3 (count (:live-worlds state))))

    (it "does not reveal roles"
      (should-not (re-find #"AGENT|AWAKE|SLEEPER|(?i)is the agent" (text output))))))
